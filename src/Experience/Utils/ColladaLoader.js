...
class ColladaLoader extends Loader {
    ...
    parse( text, path ) {
        ...
        function parseVisualSceneExtraNode( xml ) {
            ...
            const technique = xml.getElementsByTagName('technique')
            for(let i = 0; i < technique.length; i++) {
                const param = technique[i].getElementsByTagName('param')
                for(let j = 0; j < param.length; j++) {
                    const attr = param[j].getAttribute('name')
                    const value = param[j].textContent
                    data[attr] = value
                }
            }
